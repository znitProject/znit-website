...
export default function Step1({ formData, updateFormData }: Step1Props) {
  const projectTypes: ProjectType[] = [
    ...
    { value: "strategy", label: "서버전략 & 유저보수" },
  ...
  return (
    <div className="py-2 sm:py-4 md:py-6 xl:py-8">
      <h2 className="text-2xl sm:text-3xl md:text-4xl xl:text-4xl font-bold text-gray-900 mb-8 sm:mb-10 md:mb-12 xl:mb-12">
        어떤 프로젝트를 진행하실 계획인가요?
      </h2>
      <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4 xl:gap-4 justify-center sm:justify-start">
        {projectTypes.map((type, index) => (
          <motion.div
            key={type.value}
            className={`px-3 py-2 sm:px-4 sm:py-3 md:px-6 md:py-3 xl:px-8 xl:py-4 border-2 rounded-full cursor-pointer transition-all duration-200 text-xs sm:text-sm md:text-base xl:text-lg font-medium ${
