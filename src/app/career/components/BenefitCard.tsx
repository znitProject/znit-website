...
const BenefitCard: React.FC<BenefitCardProps> = ({
  ...
  return (
    ...
    >
      {/* 앞면 */}
      <motion.div
        className="absolute inset-0 bg-white rounded-3xl p-6 lg:p-8 text-center text-gray-800 backface-hidden min-h-[280px] lg:min-h-[320px] border border-gray-300 flex flex-col items-center justify-center"
        style={{
      ...
      >
        ...
        <div className="text-2xl lg:text-3xl mb-4 relative z-10">{icon}</div>
        <h3 className="text-2xl lg:text-3xl font-extrabold mb-4 relative z-10 leading-tight">
          {title}
        </h3>
        <p className="text-base lg:text-lg opacity-90 relative z-10 leading-relaxed px-2 whitespace-pre-line">
          {description}
        </p>
      </motion.div>
      ...
      >
        ...
        <h3 className="text-2xl lg:text-3xl font-extrabold mb-4 relative z-10 leading-tight text-white">
          {title}
        </h3>
        <p className="text-sm lg:text-base opacity-95 leading-relaxed px-4 relative z-10 text-white whitespace-pre-line">
          {backDescription
            ? backDescription
            : `${title}에 대한 상세한 정보를 확인하세요`}
        </p>
      </motion.div>
    </motion.div>
  );
